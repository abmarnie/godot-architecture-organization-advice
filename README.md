...
The advice is a summary of observations and not a step-by-step guide. **Tailor architecture to the individual needs of each project.** Follow recomendations selectively based on your goals, project requirements, preferences, and values. Avoid anything that seems unfun (don't feel compelled to do a massive refactor). Generally be judicious (KISS, YAGNI, 80/20 rule...).

- In a 2 day game jam, you should probably ignore architecture entirely. If your codebase is going to end up under 10k lines-of-code, your project is going to have under 100 scenes or resources, your project is to expected to only take 6 months, and/or you are working solo, the benefits here are probably marginal at best. The importance of architecture scales with the size and complexity of your project.
...
- **Tech Stack Updates**: Consider updating Godot, .NET/C#, etc., depending on the release timeline for your game. If your game is to be released in, say, only 6 months to a year from now, it probably is worth completely locking down all dependencies versions.
