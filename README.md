...
- **Data Persistence and Sharing**: Use the `static` keyword for scene-persistent data if possible (e.g. persist data for something like a Player or GUI object in a singleplayer game, or potentially even having a `static` array for multiplayer games) or shared information and functionality. Other options include [custom resources](https://docs.godotengine.org/en/stable/tutorials/scripting/resources.html#creating-your-own-resources) (though, I personally only use these for [type objects](https://gameprogrammingpatterns.com/type-object.html) or [flyweight object](https://gameprogrammingpatterns.com/flyweight.html) patterns) and autoloads (which the [Godot docs recommends you use sparingly](https://docs.godotengine.org/en/stable/tutorials/best_practices/autoloads_versus_internal_nodes.html) in large projects; as a rule of thumb, they are fine if they have no dependencies and their internal state is publically read-only).
