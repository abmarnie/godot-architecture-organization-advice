...
- **Search-Based Navigation**: Prefix resources names exclusively-used by a specific scene filenames that scene's name for efficient searching. Example: searching "balls_fish" should locate `balls_fish.tscn` and it's exclusive resources and files `balls_fish.gltf`, `balls_fish_albedo.png`, `balls_fish.mesh` `balls_fish_fishdata.tres`, etc. In general, consider searchability when naming new files.
